...
class Tasks extends React.Component {
  ...
  render() {
    ...
    const myTasks = tasks.filter(
      (task) => task.assigned_to === id && task.status === "open"
    );
    ...
    const personalCompletedTasks = tasks.filter(
      (task) => task.assigned_to === id && task.status === "completed"
    );
      ...
      <div className="pageContainer">
        <div className="task-btn-nav">
            ...
            <button
              ...
              onClick={() =>
                this.props.history.push(
                  `${this.props.match.url}/householdtasks`
                )
              }
            >
              Household Tasks
          ...
          <button
            className="btn-primary"
            onClick={() =>
              this.props.history.push(`${this.props.match.url}/completedtasks`)
            }
          >
            Completed Tasks
          </button>
        ...
        <Switch>
          <Route
            ...
            render={(routeProps) => (
              <MyTasks
                {...routeProps}
                tasks={myTasks}
                populateTasks={() => this.populateTasks(household_id)}
              />
            )}
          />
          <Route
            exact
            path={`${this.props.match.path}/householdtasks`}
            render={(routeProps) => (
              <HouseholdTasks
                {...routeProps}
          ...
          <Route
            exact
            path={`${this.props.match.path}/completedtasks`}
            render={(routeProps) => (
              <CompletedTasks
                {...routeProps}
                tasks={completedTasks}
                personalTasks={personalCompletedTasks}
